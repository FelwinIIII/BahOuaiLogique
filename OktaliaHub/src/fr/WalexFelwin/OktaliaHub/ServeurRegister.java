...
public class ServeurRegister {
	...
	public static void RegisterServeur(){
		...
		ServerInfo Faction = fr.WalexFelwin.OktaliaAPI.Bukkit.Main.getInstance().getPingAPI().addServer("Faction", ""+SERVEUR_VPS1+"", 25590, 4000);
		Faction.pingToServer();
		ServerInfo Minage = fr.WalexFelwin.OktaliaAPI.Bukkit.Main.getInstance().getPingAPI().addServer("Minage", ""+SERVEUR_VPS1+"", 25591, 4000);
		Minage.pingToServer();
