...
public class API_OktaCoins {
	  ...
	  public static float getoktacoinsof(String uuid)
	  {
		  float c = 0;
	    try
	    {
	      ResultSet rs = Update.Query("SELECT `OktaCoins` FROM `InfoOktaCoins` WHERE `UUID`='" + uuid + "'");
	      while (rs.next()) {
	        c = rs.getFloat(1);
	      }
	    }
	  ...
	  public static void removeoktacoinsoffline(String p, float c2)
	  {
		  float c = getoktacoinsof(p);
	    c -= c2;
	    Update.Update("UPDATE `InfoOktaCoins` SET `OktaCoins`='" + c + "' WHERE `UUID`='" + p + "'");
	  }
	  
	  public static void addoktacoinsoffline(String p, float coins)
	  {
		  float c = getoktacoinsof(p);
	    c += coins;
	    Update.Update("UPDATE `InfoOktaCoins` SET `OktaCoins`='" + c + "' WHERE `UUID`='" + p + "'");
	  ...
	  public static void setoktacoinsoffline(String p, float coins)
	  {
		  float c = 0;
	    c = coins;
	    Update.Update("UPDATE `InfoOktaCoins` SET `OktaCoins`='" + c + "' WHERE `UUID`='" + p + "'");
	  ...
	  public static float getBoutiquecoinsof(String uuid)
	  {
		  float c = 0;
	    try
	    {
	      ResultSet rs = Update.Query("SELECT `BoutiqueCoins` FROM `InfoOktaCoins` WHERE `UUID`='" + uuid + "'");
	      while (rs.next()) {
	        c = rs.getFloat(1);
	      }
	    }
	  ...
	  public static float getBoutiquecoins(Player p)
	  {
	    ...
	    try
	    {
	      ResultSet rs = Update.Query("SELECT `BoutiqueCoins` FROM `InfoOktaCoins` WHERE `UUID`='" + p.getUniqueId() + "'");
	      while (rs.next()) {
	        c = rs.getFloat(1);
	      }
	    }
